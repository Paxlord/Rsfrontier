...
use crate::binutils::align_up;

pub fn is_buf_momo_archive(buf: &[u8]) -> bool {
    if buf.len() < 4 {
        return false;
    }
...
pub fn decode_momo_archive(buf: &[u8]) -> Vec<Vec<u8>> {
    let mut out = Vec::new();
...
pub fn encode_momo_archive(files: &[Vec<u8>]) -> Vec<u8> {
    let out = Vec::new();
    let mut cursor = Cursor::new(out);

    let file_count = files.len();
    cursor.write_u32::<LittleEndian>(0x4F4D4F4D).unwrap(); // "MOMO" magic
    cursor.write_u32::<LittleEndian>(file_count as u32).unwrap();
    let header_end = (8 + file_count * 8) as u32;

    let mut file_start_off = align_up(header_end + 32, 16);
    ...
    let padding_needed = align_up(header_end + 32, 16) - current_pos;
    cursor
